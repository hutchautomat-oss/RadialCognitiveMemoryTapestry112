...
The non-obvious choices a reader couldn't infer from the code. Full rationale — byte layout, acuity-budget math, and the two-constants warning — in [`docs/architecture-decisions.md`](./docs/architecture-decisions.md). Load-bearing one-liners:

- **28-byte CRVM stride** `[nodeIndex u16][intentId u16][x,y,z,scale f32][lwwTimestamp f64]`; `intentId` (bytes 2-3) reserved, currently broadcast as 0.
- **Unified 3D Fibonacci sphere** — position is a deterministic function of slot index (one 137.508° spiral, radius `sqrt(slot)*0.6`); contiguous tier ranges yield foveated shells with no per-tier radius table.
- **Five-tier ontology by color + radial band, not Z.** Never conflate the removed `5.0` Z-stride (deleted render decoration) with the `10000.0` cleartext-matrix scale (cloud-side pre-CKKS, never reaches the viewport).
- **8,000-node hard cap**, single InstancedMesh / draw call, 224 KB, O(1) dead-slot recycle ("8k kill-switch"). **Local-only ONNX inference** (text never leaves the machine). **Server-arbitrated LWW** (flat timestamps, no vector clocks). **Binary frame playback** (scrub swaps frame index; live = `mockFrames[0]` mutated in place). **BVH `maxLeafTris:1`**, lazy rebuild. **Semantic saccade `/find` is read-only retrieval**, never placement.
- **Day-1 vs current:** position once encoded 3 labeled semantic axes; commit `8767217` pivoted to foveated shells — don't reinvent the axes. History: [`docs/day1-vs-current.md`](./docs/day1-vs-current.md).
