...
| `.rcmt` binary parser — `@workspace/rcmt-parser`                     | `lib/rcmt-parser/src/index.ts`; `parse()`, `findRelations()`, `toJSON/toTypeScript/toPython/toSummary/toSVG/toBinary()`; 48 vitest invariants in `lib/rcmt-parser/src/parser.test.ts` |
| `rcmt-parse` CLI — decode any `.rcmt` file to JSON/TS/Python/summary/SVG/binary | `lib/rcmt-parser/bin/rcmt-parse.ts`; six format flags (`--json/--ts/--py/--summary/--svg/--bin`) + `--relations`; stdin (`-`) and file path; exit-code contract |
| `.rcmt` lattice SVG render — top-down foveal-gradient projection      | `toSVG()` in `lib/rcmt-parser/src/index.ts`; one circle per occupied slot, positioned by (x,y), sized by `scale`, colored by `TIER_RGB`; `rcmt-parse --svg` |
| `.rcmt` raw-binary re-encode — `parse() -> toBinary() -> parse()` fixed point | `toBinary()` in `lib/rcmt-parser/src/index.ts`; `rcmt-parse --bin` |
